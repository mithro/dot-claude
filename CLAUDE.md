...
- **NEVER** use `git push --force` or `git push --force-with-lease` directly. The `permissions.deny` list in `settings.json` enforces this and will **deny** the command. **ALWAYS** use the safe wrapper commands that require explicit branch names.
...
- **NEVER** redirect stderr to `/dev/null` (i.e. `2>/dev/null`). A PreToolUse hook enforces this and will **deny** the command. Stderr output contains valuable diagnostic information — errors, warnings, and unexpected conditions — that must always be visible. Suppressing it hides problems and makes debugging harder.
